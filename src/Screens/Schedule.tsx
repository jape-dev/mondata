...
export const ScheduleTable: React.FC<ScheduleProps> = ({
  sessionToken,
  user,
}) => {
  ...
  const fetchSchedules = async () => {
    try {
      if (sessionToken) {
        setLoading(true);
        const fetchedSchedules = await RunService.runGetUserSchedules(
          sessionToken
        );
  ...
  const cronToHuman = (
    ...
    tz_offset: number | null | undefined
  ) => {
    if (
      cron === null ||
      cron === undefined ||
  ...
  const getTime = (minute: string, hour: string, tz_offset: number) => {
    ...
    return `${localHour.toString().padStart(2, "0")}:${minute.padStart(
      2,
      "0"
    )}`;
  };
  ...
  return (
    <div className="p-2">
      <Table
        ...
        emptyState={
          schedules.length === 0 ? <div>No schedules found</div> : null
        }
        columns={columns}
      >
        <TableHeader>
        ...
        <TableBody>
          {schedules.map((schedule) => (
            <TableRow key={schedule.id}>
              ...
              <TableCell>
                {schedule.connector ? (
                  <div className="flex items-center">
                    ...
                    />
                    <p className="text-sm">
                      {getConnectorName(schedule.connector)}
                    </p>
                  </div>
                ) : (
                  "-"
              ...
              <TableCell>
                {cronToHuman(schedule.cron, schedule.tz_offset)}
              </TableCell>
